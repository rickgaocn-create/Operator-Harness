...
└── windows/
    ...
    └── global-hooks/                       ← the hook SCRIPTS (copies of ~/.claude/hooks/*)
        ...
        └── RG-signal-*.cmd / RG-watcher-clippings.cmd  (Task Scheduler shims)
```
...
1. Clone the vault to `{{VAULT_ROOT}}` (adjust paths below if different).
...
- **Windows:** `{{PYTHON_EXE}} {{VAULT_ROOT}}/.claude/hooks/cn-zero-english-guard.py`
