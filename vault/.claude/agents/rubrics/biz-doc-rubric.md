...
| H4 | **User-name routing** | CN/JP docs use `{{USER_NAME}}` or role-based reference (`BD负责人`, `プロジェクトリード`). EN/casual cross-border may use `{{USER_NAME}}` | `{{USER_NAME}}` in CN/JP work body |
