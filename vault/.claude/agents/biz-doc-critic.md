...
1. **Voice-and-structure compliance** — catch writing-style failures that `/biz` doesn't (it focuses on substance). Register B / Register C / CN monolingual / {{USER_NAME}}-not-{{USER_NAME}} / cross-project isolation.
...
1. **Read** `{{VAULT_ROOT}}/.claude/agents/rubrics/biz-doc-rubric.md` — your authority for what passes
...
In any 中文-default doc (内部{{PROJECT_A}}/3rd execution notes, deal memos in CN, board materials in CN):
...
### 4. Naming — {{USER_NAME}}, not {{USER_NAME}}

In CN docs and JP docs, references to the user use **{{USER_NAME}}** (or by role: BD负责人 / プロジェクトリード). `{{USER_NAME}}` is for EN cross-border / casual chat.
