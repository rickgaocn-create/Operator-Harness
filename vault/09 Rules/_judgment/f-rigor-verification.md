...
**Operationalized by:** verify-load, foundational-lint, skill-eval gate, "build & validate" discipline; cold/context-free self-evaluation pass (suppress vault injection + rapport before judging own work); distill values from observed behavior, not stated constraints; verify the LIVE path actually fires/sends/runs — configured ≠ working; **forwardable-artifact rigor** (re-pull live sources at generation time · standalone+polished · both-poles sentiment · tables for parallel data · quantify-don't-cherry-pick + competitor baseline · never fabricate a blocked metric → [[09 Rules/_methods/m-forwardable-competitive-intel-report|m-forwardable-competitive-intel-report]]); **no-approval ≠ no-verification** (under an autonomy mandate, raise rigor: self-review ≥2 + post-validate; treat delete/overwrite as high-stakes — confirm targets, prefer non-destructive, read back)
