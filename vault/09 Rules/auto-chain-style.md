...
| 标识 | self / vault 内部 | 项目内 PMO（诸葛 / 小K 已知 chain） | **公司其他成员（{{PERSON}} / 财务 / 法务 / 外部）** |
|---|---|---|---|
...
**Driver**：2026-05-28 周报 v2 给{{PERSON}} audience 时仍保留 `T260526-channel-resource-push` 等代号被 push-back —「考虑到受众是我公司其他成员类似百洋」。

**为什么强制载体无关**：path-based guard 只保护文件落盘，但项目内沟通有大量 Feishu DM / 群消息形式的"轻文档"，audience 仍是诸葛 / {{PERSON}} / 内部 PMO — 同等正式度要求。语义类触发把守护从"文件类"延伸到"任何 report 类输出"。
...
- ❌ {{USER_NAME}} → ✅ **{{USER_NAME}}**（per MEMORY.md `feedback_cn-monolingual-and-name`）
- ❌ "{{USER_NAME}} own" → ✅ "{{USER_NAME}} 负责"
...
- **专有名词与品牌：** TapTap · B 站 / Bilibili · {{PROJECT_A}} · 心动 · {{ORG_A}} · {{ORG_D}} · ByteDance / 字节跳动 · Nuverse · HoYoverse / 米哈游 · SEGA · 任天堂 · Anthropic · OpenAI · Google · Apple
...
**判断准则：** 这条短语在你{{PERSON}} / 诸葛 / 何宗寰 实际说话中会自然出现吗？如果不会，就是 AI 套话，必须替换。
...
✗ section header 绑 specific person + title:
  · 「商务总监李响」· 「{{USER_NAME}} 负责」· 「{{PROJECT_A}}主创 负责（{{USER_NAME}}转达）」· 「TapTap 负责（仅记录）」
...
```
✗ 「李响（商务总监 / 召集人）」  →  ✓ 「李响」
✗ 「{{PROJECT_A}}主创 负责（{{USER_NAME}}转达）」 →  ✓ 「{{PROJECT_A}}侧」
